...
def parse_to_ist(series):
    """
    Sirf date part extract karo — time ane timezone bilkul ignore karo.
    "2026-03-18T23:06:54-05:00"  ->  2026-03-18
    ...
    for val in series:
        ...
        try:
            ...
            # ISO format: "2026-03-18T..." — pehla 10 char = YYYY-MM-DD
    ...
    with st.spinner(""):
        try:
            ...
            for sname in found_spreadsheets:
                ...
                for project_name in sdf['Project'].unique():
                    pdf_df = sdf[sdf['Project'] == project_name].copy()
                    pdf_df = pdf_df.drop(columns=['created_dt', '_spreadsheet', '_created_time_raw'], errors='ignore').reset_index(drop=True)
                    for col in pdf_df.columns:
                        pdf_df[col] = pdf_df[col].astype(str).replace(
                            {'None': '', 'nan': '', 'NaT': '', 'none': '', 'NaN': ''})
                    if 'Project' in pdf_df.columns:
